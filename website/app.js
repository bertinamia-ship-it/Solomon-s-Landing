...
function renderMenu(category) {
    ...
    Object.keys(itemsByCategory).forEach(catName => {
        // Agregar encabezado de categoría (excepto para 'main') - Holiday Edition Compacto
        if (catName !== 'main') {
            menuHTML += `
                <div class="menu-category-header" style="margin: 2.5rem 0 1.5rem 0; padding: 1rem 1.5rem; background: linear-gradient(135deg, rgba(22, 91, 51, 0.08), rgba(196, 30, 58, 0.08)); border-radius: 12px; border-left: 4px solid var(--holiday-red); box-shadow: 0 3px 12px rgba(0,0,0,0.05); position: relative; overflow: hidden;">
                    <div style="position: absolute; top: 0.4rem; right: 0.8rem; font-size: 1.2rem; opacity: 0.12;">✨</div>
                    <h3 style="font-size: 1.7rem; color: var(--holiday-green); font-weight: 800; text-transform: uppercase; letter-spacing: 1.2px; margin: 0;">${catName}</h3>
                </div>
            `;
        }
        ...
        menuHTML += itemsByCategory[catName].map(item => {
            ...
            return `
                <div class="menu-item" style="background: linear-gradient(to bottom, #ffffff 0%, #fafafa 100%); padding: 2rem; border-radius: 16px; box-shadow: 0 6px 25px rgba(0,0,0,0.1); transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); border-left: 5px solid transparent; position: relative; overflow: hidden;">
